...
class graph_v_of_v_with_time_span_hop_constrained : public graph_v_of_v_with_time_span
{
private:
public:
	graph_v_of_v_with_time_span_hop_constrained() : graph_v_of_v_with_time_span() {};
	graph_v_of_v_with_time_span_hop_constrained(int n, int e, weight_type weight_upper_limit, weight_type weight_lower_limit, int time_max) : graph_v_of_v_with_time_span(n, e, weight_upper_limit, weight_lower_limit, time_max) {};
	 ...
	 * 	@endTime
	 */
	weight_type search_shortest_path_in_period_time_naive(int u, int v, int k, int startTime, int endTime)
	{
	...
	vector<graph_v_of_v<weight_type>> graph_v_of_v_generate_random_graph_with_same_edges_of_different_weight(int change_num, int decrease_time, int increase_time, float change_ratio, hop_constrained_case_info& case_info, hop_constrained_case_info& case_info_2021)
	{
		...
		boost::random::uniform_int_distribution<> dis(0, this->v_num);
		int index = 1;
		...
		while (index <= change_num)
		{
			...
			while (current_decrease_time > 0)
			{
				...
				boost::random::uniform_int_distribution<> dis_inner(0, instance_graph.ADJs[i].size() - 1);
	...
	vector<graph_v_of_v<weight_type>> txt_read(std::string save_name, hop_constrained_case_info& case_info, hop_constrained_case_info& case_info_2021)
	{
		this->clear();
